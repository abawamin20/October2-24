...
import { Dialog, DialogFooter, PrimaryButton, Spinner } from "@fluentui/react";
...
const PagesList = (props: IPagesListProps) => {
  ...
  const [isDateVisible, setIsDateVisible] = React.useState<boolean>(false);
  // The search text for filtering pages
  const [searchText, setSearchText] = React.useState<string>(""); // Initially set to empty string
  const [startDate, setStartDate] = React.useState<string>(""); // Initially set to empty string
  const [endDate, setEndDate] = React.useState<string>(""); // Initially set to empty string
  ...
  const [showDateRangeSelections, setShowDateRangeSelections] =
    ...
    const checkIframeUrl = () => {
      ...
      if (iframe && iframe.contentWindow) {
        ...
        if (
          ...
          setHasNextPage(true);
          setNextPageUrl(null);
          setShowDateRangeSelections(false);
          setStartDate("");
          setEndDate("");
    ...
    return (
      ...
      pagesService
        .getFilteredPages(
          ...
          showDateRangeSelections ? startDate : null,
          showDateRangeSelections ? endDate : null
        )
        .then((res: SuccessResponse | ErrorResponse) => {
          if (res.hasOwnProperty("error")) {
            setShowDateRangeSelections(true);
            setTotalItems(0);
            setPages([]);
            ...
            let { pages, fetchednextPageUrl } = res as SuccessResponse;
        ...
        {showDateRangeSelections && (
          <div className="d-flex justify-content-center align-items-center mt-2">
            <input
              ...
              style={{
                color: isDateVisible ? "initial" : "transparent", // Control visibility of text
              }}
              className="me-2"
            />
