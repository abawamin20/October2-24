    ...
    try {
      ...
      if (nextPageUrl) {
        ...
        if (startDate && endDate) {
          const dateQuery = `  and Modified ge datetime'${new Date(
            startDate
          ).toISOString()}' and Modified le datetime'${new Date(
            endDate
          ).toISOString()}'`;

          filterQuery = `KnowledgeBaseLabel eq '${category}' and FSObjType eq 0${dateQuery}${
            searchText
              ? ` and (substringof('${searchText}', Title) or Article_x0020_ID eq '${searchText}' or substringof('${searchText}', Modified))`
              : ""
          }`;
        } else {
          filterQuery = `KnowledgeBaseLabel eq '${category}' and FSObjType eq 0${
            searchText
              ? ` and (substringof('${searchText}', Title) or Article_x0020_ID eq '${searchText}' or substringof('${searchText}', Modified))`
              : ""
          }`;
      ...
      if (jsonResponse.hasOwnProperty("error")) {
        ...
        return {
          ...
          fetchednextPageUrl: nextPageLink,
        };
      }
    } catch (error) {}
  };
